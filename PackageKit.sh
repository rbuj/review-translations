...
function usage {
    echo "This script downloads the translation of the PackageKit package"
...
function get_code {
    download_code PackageKit https://github.com/hughsie/PackageKit.git
}
...
function report {
    ...
    HTML_REPORT=${WORK_PATH}/PackageKit-report.html
    cat << EOF > ${HTML_REPORT}
<!DOCTYPE html>
...
EOF
    ...
    echo "************************************************"
    posieve check-rules,check-spell-ec,check-grammar,stats -s lang:${LANG_CODE} -s showfmsg -s byrule --msgfmt-check --skip-obsolete --coloring-type=html ${BASE_PATH}/PackageKit/po/${LANG_CODE}.po >> ${HTML_REPORT}
...
function install {
    ...
    set -x
    sudo rm -f /usr/share/locale/${LANG_CODE}/LC_MESSAGES/PackageKit.mo
    sudo msgfmt ${WORK_PATH}/PackageKit/po/${LANG_CODE}.po -o /usr/share/locale/${LANG_CODE}/LC_MESSAGES/PackageKit.mo
    set -
}
