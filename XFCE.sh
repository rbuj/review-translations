...
if [ -n "$GENERATE_REPORT" ]; then
    if [ "$DISABLE_WORDLIST" == "YES" ]; then
        ./common/git-translations.sh -r -l=${LANG_CODE} --disable-wordlist -p=${PROJECT_NAME} -f=${LIST} -w=${WORK_PATH}
    else
        ./common/git-translations.sh -r -l=${LANG_CODE} -p=${PROJECT_NAME} -f=${LIST} -w=${WORK_PATH}
    fi
fi
if [ -n "$INSTALL_TRANS" ]; then
    ./common/git-translations.sh -i -l=${LANG_CODE} -p=${PROJECT_NAME} -f=${LIST} -w=${WORK_PATH}
fi
echo "complete!"
