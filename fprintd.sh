...
function usage {
    echo "This script downloads the translation of the fprintd package"
...
function get_code {
    download_code fprintd git://anongit.freedesktop.org/libfprint/fprintd
}
...
function report {
    ...
    HTML_REPORT=${WORK_PATH}/fprintd-report.html
    cat << EOF > ${HTML_REPORT}
<!DOCTYPE html>
...
EOF
    ...
    echo "************************************************"
    posieve check-rules,check-spell-ec,check-grammar,stats -s lang:${LANG_CODE} -s showfmsg -s byrule --msgfmt-check --skip-obsolete --coloring-type=html ${BASE_PATH}/fprintd/po/${LANG_CODE}.po >> ${HTML_REPORT}
...
function install {
    ...
    set -x
    sudo rm -f /usr/share/locale/${LANG_CODE}/LC_MESSAGES/fprintd.mo
    sudo msgfmt ${WORK_PATH}/fprintd/po/${LANG_CODE}.po -o /usr/share/locale/${LANG_CODE}/LC_MESSAGES/fprintd.mo
    set -
}
