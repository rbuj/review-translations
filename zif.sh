...
function usage {
    echo "This script downloads the translation of the zif package"
    ...
    echo ""
    echo "Zif is now a dead project. See http://people.freedesktop.org/~hughsient/zif/"
    echo ""
}
...
function get_code {
    download_code zif https://github.com/hughsie/zif.git
}
...
function get_trans {
    ...
    sed -i 's#http://www.transifex.net#https://www.transifex.com#g' .tx/config
...
function report {
    ...
    HTML_REPORT=${WORK_PATH}/zif-report.html
    cat << EOF > ${HTML_REPORT}
<!DOCTYPE html>
...
EOF
    ...
    echo "************************************************"
    posieve check-rules,check-spell-ec,check-grammar,stats -s lang:${LANG_CODE} -s showfmsg -s byrule --msgfmt-check --skip-obsolete --coloring-type=html ${BASE_PATH}/zif/po/${LANG_CODE}.po >> ${HTML_REPORT}
...
function install {
    ...
    set -x
    sudo rm -f /usr/share/locale/${LANG_CODE}/LC_MESSAGES/zif.mo
    sudo msgfmt ${WORK_PATH}/zif/po/${LANG_CODE}.po -o /usr/share/locale/${LANG_CODE}/LC_MESSAGES/zif.mo
    set -
}
