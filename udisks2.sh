...
function usage {
    echo "This script downloads the translation of the udisks2 package"
...
function get_code {
    download_code udisks2 http://anongit.freedesktop.org/git/udisks.git
}
...
function report {
    ...
    HTML_REPORT=${WORK_PATH}/udisks2-report.html
    cat << EOF > ${HTML_REPORT}
<!DOCTYPE html>
...
EOF
    ...
    echo "************************************************"
    posieve check-rules,check-spell-ec,check-grammar,stats -s lang:${LANG_CODE} -s showfmsg -s byrule --msgfmt-check --skip-obsolete --coloring-type=html ${BASE_PATH}/udisks2/po/${LANG_CODE}.po >> ${HTML_REPORT}
...
function install {
    ...
    set -x
    sudo rm -f /usr/share/locale/${LANG_CODE}/LC_MESSAGES/udisks2.mo
    sudo msgfmt ${WORK_PATH}/udisks2/po/${LANG_CODE}.po -o /usr/share/locale/${LANG_CODE}/LC_MESSAGES/udisks2.mo
    set -
}
