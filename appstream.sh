...
function usage {
    echo "This script downloads the translation of the AppStream"
...
function get_code {
    download_code appstream https://github.com/ximion/appstream
}
...
function report {
    ...
    HTML_REPORT=${WORK_PATH}/appstream-report.html
    cat << EOF > ${HTML_REPORT}
<!DOCTYPE html>
...
EOF
    ...
    echo "************************************************"
    posieve check-rules,check-spell-ec,check-grammar,stats -s lang:${LANG_CODE} -s showfmsg -s byrule --msgfmt-check --skip-obsolete --coloring-type=html ${BASE_PATH}/appstream/po/${LANG_CODE}.po >> ${HTML_REPORT}
...
function install {
    ...
    set -x
    sudo rm -f /usr/share/locale/${LANG_CODE}/LC_MESSAGES/appstream.mo
    sudo msgfmt ${WORK_PATH}/appstream/po/${LANG_CODE}.po -o /usr/share/locale/${LANG_CODE}/LC_MESSAGES/appstream.mo
    set -
}
